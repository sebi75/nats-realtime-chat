...
CREATE TABLE `ChatPreview` (
    ...
    `lastMessageText` VARCHAR(191) NULL,
    `lastMessageSentAt` DATETIME(3) NULL,
    `lastMessageSentById` VARCHAR(191) NULL,

    UNIQUE INDEX `ChatPreview_chatId_key`(`chatId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `ChatPreview` ADD CONSTRAINT `ChatPreview_chatId_fkey` FOREIGN KEY (`chatId`) REFERENCES `Chat`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
